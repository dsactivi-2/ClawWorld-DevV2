 ...
 * Set API_URL to a reachable server to enable these tests; otherwise this file is skipped.
 *
 * Run with:
...
const RUN_E2E = Boolean(process.env['API_URL']);
...
async function safeRequest<T>(fn: () => Promise<T>): Promise<T> {
  try {
    ...
    // Axios errors can contain circular references (http.Agent) which can crash Jest worker IPC.
    throw new Error(`E2E request failed (API_URL=${API_URL}): ${getAxiosErrorMessage(err)}`);
  }
}

const describeE2E = RUN_E2E ? describe : describe.skip;

describeE2E('E2E: Health & Metrics', () => {
  it('GET /health returns 200 with status healthy or degraded', async () => {
    const res = await safeRequest(() => client.get('/health'));
  ...
  it('GET /metrics returns 200 with Prometheus exposition text', async () => {
    const res = await safeRequest(() => client.get('/metrics'));
    expect(res.status).toBe(200);
...
describeE2E('E2E: Workflows', () => {
  ...
  it('POST /api/workflows returns 400 when userInput is missing', async () => {
    const res = await safeRequest(() => client.post('/api/workflows', {}));
    expect(res.status).toBe(400);
    expect(res.data).toHaveProperty('error');
  });

  it('POST /api/workflows returns 400 when userInput is empty string', async () => {
    const res = await safeRequest(() => client.post('/api/workflows', { userInput: '' }));
    expect(res.status).toBe(400);
  });

  it('GET /api/workflows returns 200 with an items array', async () => {
    const res = await safeRequest(() => client.get('/api/workflows'));
    expect(res.status).toBe(200);
  ...
  it('GET /api/workflows supports pagination with page and pageSize', async () => {
    const res = await safeRequest(() => client.get('/api/workflows?page=1&pageSize=5'));
    expect(res.status).toBe(200);
  ...
  it('GET /api/workflows/:id returns 200 with workflow details', async () => {
    ...
    const res = await safeRequest(() => client.get(`/api/workflows/${createdWorkflowId}`));
  ...
  it('GET /api/workflows/:id returns 404 for nonexistent id', async () => {
    const res = await safeRequest(() => client.get('/api/workflows/nonexistent-id-12345'));
    expect(res.status).toBe(404);
  });

  it('GET /api/workflows/:id/graph returns mermaid diagram or 404', async () => {
    if (!createdWorkflowId) return;
    const res = await safeRequest(() => client.get(`/api/workflows/${createdWorkflowId}/graph`));
    expect([200, 404]).toContain(res.status);
    if (res.status === 200) {
  ...
  it('DELETE /api/workflows/:id cancels the workflow and returns 200 or 404', async () => {
    if (!createdWorkflowId) return;
    const res = await safeRequest(() => client.delete(`/api/workflows/${createdWorkflowId}`));
    expect([200, 202, 404]).toContain(res.status);
  });
});

describeE2E('E2E: Agents', () => {
  it('GET /api/agents returns 200 with an agents array', async () => {
    const res = await safeRequest(() => client.get('/api/agents'));
    expect(res.status).toBe(200);
  ...
  it('GET /api/agents/:id returns 404 for nonexistent agent', async () => {
    const res = await safeRequest(() => client.get('/api/agents/nonexistent-agent-00001'));
    expect(res.status).toBe(404);
  });

  it('POST /api/agents/:id/task returns 404 for unknown agent', async () => {
    const res = await safeRequest(() =>
      client.post('/api/agents/no-agent/task', { task: 'do something' }),
    );
    expect(res.status).toBe(404);
  });

  it('GET /api/agents/:id/metrics returns 404 for unknown agent', async () => {
    const res = await safeRequest(() => client.get('/api/agents/no-agent/metrics'));
    expect(res.status).toBe(404);
  });
});

describeE2E('E2E: Teams', () => {
  let spawnedTeamId: string;

  it('POST /api/teams/spawn creates a team and returns 201', async () => {
    const res = await safeRequest(() =>
      client.post('/api/teams/spawn', {
        name: 'E2E Test Team',
  ...
  it('GET /api/teams returns 200 with a teams array', async () => {
    const res = await safeRequest(() => client.get('/api/teams'));
    expect(res.status).toBe(200);
  ...
  it('GET /api/teams/:id returns team health for a spawned team', async () => {
    ...
    const res = await safeRequest(() => client.get(`/api/teams/${spawnedTeamId}`));
    expect([200, 404]).toContain(res.status);
    if (res.status === 200) {
  ...
  it('POST /api/teams/:id/scale returns 200 when scaling the team', async () => {
    if (!spawnedTeamId) return;
    const res = await safeRequest(() =>
      client.post(`/api/teams/${spawnedTeamId}/scale`, { targetCount: 2 }),
    );
    expect([200, 404]).toContain(res.status);
  });
  ...
  it('DELETE /api/teams/:id despawns a team', async () => {
    if (!spawnedTeamId) return;
    const res = await safeRequest(() => client.delete(`/api/teams/${spawnedTeamId}`));
    expect([200, 202, 404]).toContain(res.status);
  });

  it('GET /api/teams/:id returns 404 for nonexistent team', async () => {
    const res = await safeRequest(() => client.get('/api/teams/ghost-team-99999'));
    expect(res.status).toBe(404);
  });
});

// ---------------------------------------------------------------------------
// Legacy blocks kept below were migrated to describeE2E + safeRequest above.
// ---------------------------------------------------------------------------
